...
def generate_management_report(
    ...
    _add_final_decision(doc, final_decision, metrics, risk_data, scenario_data)
...
def _add_fx_section(doc, fx_data):
    _add_heading(doc, "FX Exchange Rate Analysis")
...
def _add_final_decision(doc, final_decision, metrics, risk_data, scenario_data):
    _add_heading(doc, "17. Final Investment Decision")
...
def _add_recommendations(doc, final_decision, metrics):
    _add_heading(doc, "19. Recommendations")
...
def _add_limitations(doc):
    _add_heading(doc, "20. Limitations")
    limitations = [
